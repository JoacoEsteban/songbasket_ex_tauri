defmodule SongbasketExTauriWeb.PageLive do
  use SongbasketExTauriWeb, :live_view
  alias SongbasketExTauri.{Api, PubSub, Flow, Events}
  alias SongbasketExTauriWeb.Components.{Basket, Home, NewBasket}

  def update_basket(socket) do
    socket
    |> assign(selected_basket: SongbasketExTauri.Flow.get_selected_basket())
  end

  def update_baskets(socket) do
    socket
    |> assign(baskets: SongbasketExTauri.Flow.get_baskets())
  end

  def fetch_basket(socket) do
    socket
    |> assign(basket: SongbasketExTauri.Basket.get_basket())
  end
